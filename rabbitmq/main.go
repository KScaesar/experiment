...
func main() {
	...
	pool := NewPool(3, 5, func() (*AmqpConnection, error) {
		return NewAmqpConnection("amqp://guest:guest@localhost:5672/")
	})
...
func syncUseCase(pool *ConnectionPool, maxWorker int) {
	go func() {
...
func asyncUseCase(pool *ConnectionPool, maxWorker int) {
	for i := 0; i < maxWorker; i++ {
		i := i
		go func() {
			user := "user" + strconv.Itoa(i)
...
func SubscribeNotificationByUser(user string, pool *ConnectionPool) {
	...
	_, err1 := channel.CreateQueue(SetupDefaultQueue(ex1, ex1Type, queue, key1), SetupTemporaryQueue(ttl))
	if err1 != nil {
		log.Fatalf("%v", err)
	}
	...
	_, err2 := channel.CreateQueue(SetupDefaultQueue(ex2, ex2Type, queue, key2), SetupTemporaryQueue(ttl))
	if err2 != nil {
		log.Fatalf("%v", err2)
	}
	...
	ex3Type := "direct"
	key3 := user
	_, err3 := channel.CreateQueue(SetupDefaultQueue(ex3, ex3Type, queue, key3), SetupTemporaryQueue(ttl))
	if err3 != nil {
		log.Fatalf("%v", err3)
	}
