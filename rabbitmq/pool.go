...
func (p *ConnectionPool) AcquireConnection() (*Connection, error) {
	if p.done.Load() {
...
func (c *Connection) AcquireChannel() (*Channel, error) {
	c.mu.Lock()
	defer func() {
...
func (ch *Channel) CreateQueue(useParam ...UseQueueParam) (queueName string, err error) {
	// https://pkg.go.dev/github.com/streadway/amqp@v1.1.0#pkg-variables
