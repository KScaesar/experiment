...
func (s *MinUsageRateStrategy) ViewUsageQty() (minIndex, totalQty int, listQty []int) {
	s.searchMinMutex.RLock()
	defer s.searchMinMutex.RUnlock()
...
func (s *MinUsageRateStrategy) UpdateByAcquire() (targetIndex int, minIndex int) {
...
func lazyNewResource[T any](strategy *MinUsageRateStrategy, resourceAll []T, factory func(id int) (T, error)) (T, error) {
	if strategy.reachMaxLen() { // 如果先更新後查詢, 狀態判斷會有錯誤
		fmt.Println("reach")
		_, minIndex := strategy.UpdateByAcquire()
		return resourceAll[minIndex], nil
	}

	fmt.Println("lazy")
	targetIndex, _ := strategy.UpdateByAcquire()
